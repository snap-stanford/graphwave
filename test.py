# -*- coding: utf-8 -*-
"""
Created on Tue May 29 21:47:13 2018
...
import sys
sys.path.append('/Users/cdonnat/Dropbox/graphwave')
...
#time_points= np.linspace(0,100,101)
    ...
    nnz_vec = np.array([1.0/n_nodes*(temp[:,i].nnz) for i in range(n_nodes)])
    final_sig[::2,:] = np.vstack([1.0/n_nodes*np.cos(t*temp.data).sum(0) + nnz_vec for it_t, t in enumerate(time_points)])
    final_sig[1::2,:] = np.vstack([1.0/n_nodes*np.sin(t*temp.data).sum(0) for it_t, t in enumerate(time_points)])
    return final_sig
...
def graphwave2(graph, taus, time_pnts, type_graph="nx",
    ...
    if taus == 'auto':
        ...
        toc = time.time()
        print("computing range in",toc-tic)
    heat_print, _ = heat_diffusion_ind(graph, list(taus), diff_type='heat', order=30, proc = 'approximate')
...
if True:
    ...
    #np.savetxt("/dfs/scratch1/cdonnat/graphwave/log_scalable_leretour.txt", time_alg, fmt="%i, %f")
    print (toc-tic)
    
import time 
time_alg=[]
time_pts = list(np.arange(0, 100,4))
#for n_shapes in [10, 50, 100]+list(np.arange(500, 100000, 500)):
if True:
    for n_shapes in [10, 50, 100]+list(np.arange(500, 10000, 500)):
        ### 1. Choose the basis (cycle, torus or chain)
        list_shapes = [["house"]] * n_shapes
        width_basis = 5 * n_shapes 
        ...
        np.savetxt("log_scalable_leretour.txt", time_alg, fmt="%i, %f, %f, %f")
